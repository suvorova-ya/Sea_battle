...
computer_available_to_fire_set = {(x, y) for x in range(16,22)for y in range(1,7)}
...
class Grid:
    ...
    def draw_grid(self):
          for i in range(7):
            ...
            pygame.draw.line(screen, BLACK, (left_margin + self.offset * block_size, upper_margin + i * block_size),
                             (left_margin + (6 + self.offset) * block_size, upper_margin + i * block_size), 1)
            # Vertical lines
            pygame.draw.line(screen, BLACK, (left_margin + (i + self.offset) * block_size, upper_margin),
                             (left_margin + (i + self.offset) * block_size, upper_margin + 6 * block_size), 1)
    ...
    def sign_grid(self):
        ...
        screen.blit(player, (left_margin + 3 * block_size - sign_width //
...
class AutoShips:
    def __init__(self, offset):
        self.offset = offset
        self.available_blocks = {(x, y) for x in range(1 + self.offset, 7 + self.offset) for y in range(1, 7)}
        self.ships_set = set()
    ...
    def get_new_block_for_ship(self, coor, str_rev, x_or_y, ship_coordinates):
        if (coor <= 1 - self.offset * (x_or_y - 1) and str_rev == -1) or (
                coor >= 6 - self.offset * (x_or_y - 1) and str_rev == 1):
            str_rev *= -1
    ...
    def update_available_blocks_for_creating_ships(self, new_ship):
        for elem in new_ship:
            for k in range(-1, 2):
                for m in range(-1, 2):
                    if 0 + self.offset < (elem[0] + k) < 7 + self.offset and 0 < (elem[1] + m) < 7:
...
def main():
    ...
    screen.fill(WHITE)
    computer_grid = Grid("Человек", 0)
    human_grid = Grid("Компьютер", 15)
    ...
    while not game_over:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                x, y = event.pos
                if (left_margin < x < left_margin + 6 * block_size) and (
                        upper_margin < y < upper_margin + 6 * block_size):
